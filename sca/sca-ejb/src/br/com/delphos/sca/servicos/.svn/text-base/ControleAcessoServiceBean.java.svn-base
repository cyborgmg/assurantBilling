...
@WebService(
	name = "ControleAcesso",
	portName = "ControleAcessoPort",
	//serviceName = "ControleAcessoService",
	targetNamespace = ConstantesSca.NAMESPACE_SCA,
	endpointInterface = "br.com.delphos.sca.servicos.ControleAcessoService")
public class ControleAcessoServiceBean implements ControleAcessoService {
	...
	public Usuario obterPorNomeLogin(String nomeLogin) {
		...
		try {
			...
			usuario = usuarioDLO.limparDadosSessao(usuario);
	...
	public Usuario obterUsuarioPorToken(String token) {
		...
		try {
			retorno = usuarioDLO.prepararParaSerializacaoXML(
					usuarioDLO.obterPorToken(token)
					);
			retorno = usuarioDLO.limparDadosSessao(retorno);
	...
	public Usuario finalizarEsqueciSenha(String token, String hashNovaSenha) {
		...
		try {
			retorno = usuarioDLO.prepararParaSerializacaoXML(
					usuarioDLO.finalizarEsqueciSenha(token, hashNovaSenha)
					);
			retorno = usuarioDLO.limparDadosSessao(retorno);
	...
	public Usuario confirmarEmailUsuario(String token, String hashNovaSenha) {
		...
		try {
			retorno = usuarioDLO.prepararParaSerializacaoXML(
					usuarioDLO.confirmarEmailUsuario(token, hashNovaSenha)
					);
			retorno = usuarioDLO.limparDadosSessao(retorno);
