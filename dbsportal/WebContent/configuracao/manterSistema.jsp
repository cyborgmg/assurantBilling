...
<body>
				...
				<div class="tabelas">
					<table class="tabelaReadWrite">
						<tbody>
							...
							<%-- Sistema para envio de informações de arrecadação mensal ================================================================== --%>
							<tr>
								<td class="label ui-widget-header"><label><bean:message
											key="label.sistema.envio.informacoes.sistema" /></label></td>
								<td colspan="3">
									
									<div class="formataCampoSelectForm divSelectConsulta">
										<select name="sistemaEnvioInformacoesSistema" id="sistemaEnvioInformacoesSistema" 
										style="width: 100%;">
												<option value=""
												<c:choose> 
						                        <c:when test="${ConfiguracaoActionForm.sistemaEnvioInformacoesSistema == ''}">
						                            selected
						                        </c:when>
						                        </c:choose>
												 > - Selecione - </option>
												<option value="ACSEL"
												<c:choose> 
						                        <c:when test="${ConfiguracaoActionForm.sistemaEnvioInformacoesSistema == 'ACSEL'}">
						                            selected
						                        </c:when>
						                        </c:choose>
												 >ACSEL</option>
												<option value="ELITA" 
						                        <c:choose>
						                        <c:when test="${ConfiguracaoActionForm.sistemaEnvioInformacoesSistema == 'ELITA'}">
						                            selected
						                        </c:when>
						                        </c:choose>
												>ELITA</option>
	...
	<script>
    	...
    	function voltar(){
            document.pesquisaSistemaForm.method = "post";
            document.pesquisaSistemaForm.opcao.value = "listarSistema";
