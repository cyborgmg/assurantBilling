						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3">
									...
									value="${ConfiguracaoActionForm.descricaoEmpresa}" maxlength="100"
									class="parametros"/>									
								</td>
							</tr>
							<%-- ASSOCIAR PRODUTOS ================================================================== --%>
							<c:if test="${not empty produtosAssociados}" >
								<tr>
									<td class="label ui-widget-header"><label><bean:message key="label.produtos.associados" /></label></td>
									<td colspan="3">
		                              	<table class="TabelaPrin dataTable multiSelectTable" style="width: 94%" cellspacing="1">
					                        <tr>
					                            <td>
													<textarea rows="10" style="width: 96.6%; margin-left: -3px;" readonly>${produtosAssociados}</textarea>
					                            </td>
					                        </tr>
					                    </table>
									</td>
	...
	<script>
		function limpar(origem) {
			...
			if (document.pesquisaEmpresaForm.idEmpresa.value) {
				...
				//document.location = "../logs/preparaEmpresa.do";
	    ...
	    function prosseguirCB(retorno) {                
	        if (retorno){                  
	        	//mostrarMensagem("<bean:message key="msg.empresa.incluido.sucesso"/>", "dialogoSucesso", 400);       
 	            ...
 	            document.pesquisaEmpresaForm.action = "../configuracao/editarEmpresa.do?operacao=2";
    	...
    	function voltar(){
            document.pesquisaEmpresaForm.method = "post";
            document.pesquisaEmpresaForm.opcao.value = "listarEmpresa";
            document.pesquisaEmpresaForm.action = "../configuracao/resultadoEmpresa.do";
            document.pesquisaEmpresaForm.submit();   
        }
